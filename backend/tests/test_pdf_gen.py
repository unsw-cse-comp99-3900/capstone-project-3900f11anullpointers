...
#     @patch('src.pdf_gen.FPDF')
#     @patch('src.fonts.fonts.Fonts')
#     @patch('src.pdf_gen.open', read_data='{"document": "Sample document text"}')
#     @patch('src.pdf_gen.datetime')
...
#         pdf_generator.generate_pdf(client_name, form_name, signature_sample, consent_flags, timestamp)
...
#         generator = GeneratePDF()
#         generator.generate_pdf('bob marley', 'adult', signature_sample, [True, True, False], self.timestamp)
#         self.assertIsInstance(generator.pdf, FPDF)
        
#     def test_invalid_flags(self):
#         with self.assertRaises(Exception):
#             self.generator.generate_pdf('bob marley', 'adult', signature_sample, ['a', 1, 4], self.timestamp)
...
#     def test_incorrect_amount_of_flags_adult(self):
#         with self.assertRaises(Exception):
#             self.generator.generate_pdf('bob marley', 'adult', signature_sample, [False, False], self.timestamp)
        
#         with self.assertRaises(Exception):
#             self.generator.generate_pdf('bob marley', 'adult', signature_sample, [False, False, True, True], self.timestamp)
        
#     def test_incorrect_amount_of_flags_child(self):
#         with self.assertRaises(Exception):
#             self.generator.generate_pdf('bob marley', 'child', signature_sample, [False, False, True], self.timestamp)
        
#         with self.assertRaises(Exception):
#             self.generator.generate_pdf('bob marley', 'child', signature_sample, [False], self.timestamp)
    
#     def test_invalid_form_type(self):
#         with self.assertRaises(FileNotFoundError):
#             self.generator.generate_pdf('bob marley', 'lol', signature_sample, [True, True, False], self.timestamp)
    
#     def test_invalid_signature(self):
#         with self.assertRaises(Exception):
#             self.generator.generate_pdf('bob marley', 'adult', "invalid string", [False, True, True], self.timestamp)
